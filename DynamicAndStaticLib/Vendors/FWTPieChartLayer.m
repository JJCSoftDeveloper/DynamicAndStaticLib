...
- (id)init
{
  ...
  if (self != nil){
    ...
    self->_font = [UIFont fontWithName:@"HelveticaNeue" size:10.f];
...
- (void)drawInContext:(CGContextRef)ctx
{
  ...
  for (int i = 0; i < self.values.count; i++){
    ...
    if ((self.shouldDrawPercentages || [self.outerTexts[i] isEqualToString:@""] == NO) && self.animationCompletionPercent > 0/* && value >= 0.07f*/){
      ...
      if (i > 0 && !CGPointEqualToPoint(lastLimit, CGPointZero)) {
        if ((lastLimit.x > center.x) == (limit.x > center.x)) {
          if (limit.x > center.x){
            lineEnd.y = MAX(limit.y, lastLimit.y + 13);
          } else {//右边
            lineEnd.y = MIN(limit.y, lastLimit.y - 13);
          }
        }
      }
      ...
      if (limit.x > center.x){
        CGContextAddLineToPoint(ctx, lineEnd.x+horizontalLineLength, lineEnd.y);
      }
      else{
        CGContextAddLineToPoint(ctx, lineEnd.x-horizontalLineLength, lineEnd.y);
      }
    ...
    if (self.animationCompletionPercent > 0.f){
      ...
      if (limit.x > center.x){
        ...
        if (i > 0 && !CGPointEqualToPoint(lastLimit, CGPointZero)) {
          textPoint.y = MAX(textPoint.y, (lastLimit.y - verticalOffset) + 13);
        }
      }
      else{
        textPoint = CGPointMake(limit.x-horizontalLineLength-outerTextSize.width-5.f, limit.y-verticalOffset);
        if (lastLimit.x > center.x) {//right
          
        }else{//left
          if (i > 0 && !CGPointEqualToPoint(lastLimit, CGPointZero)) {
            textPoint.y = MIN(textPoint.y, (lastLimit.y - verticalOffset) - 13);
          }
        }
      }
      ...
      [self.outerTexts[i] drawAtPoint:textPoint
                       withAttributes:[self _outerLetterAttributesWithSpaceAvailable:outerSpaceAvailable andTextColor:[UIColor darkGrayColor]]];
...
{
  return self.font;
  //    return [UIFont fontWithName:self.font.fontName size:spaceAvaliable*0.25f];
}
...
{
  return self.font;
  //    return [UIFont fontWithName:self.font.fontName size:spaceAvaliable*0.28f];
}
...
{
  return self.font;
  //    return [UIFont fontWithName:self.font.fontName size:spaceAvaliable*0.15f];
}
