...
- (void)setTransform:(CGAffineTransform)transform {
  ...
  FWTPieChartLayer *pieLayer = (FWTPieChartLayer*)self.containerLayer.sublayers.firstObject;
...
- (void)reloadAnimated:(BOOL)animated withCompletionBlock:(void(^)())completionBlock
{
    ...
    pieLayer.animationCompletionPercent = 0;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(nullable UIEvent *)event {
  ...
  FWTPieChartLayer *layer = (FWTPieChartLayer *)self.containerLayer.sublayers.firstObject;
  point = [self.layer convertPoint:point toLayer:layer];
  if (layer) {
    float floatPI = 3.141592653f;
    CGRect rect = layer.frame;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    
    __block float startAngle = layer.startAngle;
    float maxAngle = (floatPI*2);
    float outerRadius = ((float)CGRectGetMidY(rect))*0.65f;
    
    [[self _values] enumerateObjectsUsingBlock:^(NSNumber *obj, NSUInteger idx, BOOL *stop) {
      float value = obj.floatValue;
      float segmentAngle = maxAngle*value*layer.animationCompletionPercent;
      ...
      if (CGPathContainsPoint(path, NULL, point, NO)) {
        NSLog(@"point:(%f, %f, idx:%lu), Touch arc.", point.x, point.y, (unsigned long)idx);
        if ([self.delegete respondsToSelector:@selector(FWTPieChartView:didTouchAtSegment:atIndex:)]) {
          [self.delegete FWTPieChartView:self didTouchAtSegment:self.segments[idx] atIndex:idx];
          *stop = YES;
        }
      }
      CGPathRelease(path);
