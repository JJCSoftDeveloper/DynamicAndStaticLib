...
void TBFreeQuadTreeNode(TBQuadTreeNode* node, TBDataReleaseBlock releaseBlock)
{
  if (node->northWest != NULL) TBFreeQuadTreeNode(node->northWest, releaseBlock);
  if (node->northEast != NULL) TBFreeQuadTreeNode(node->northEast, releaseBlock);
  if (node->southWest != NULL) TBFreeQuadTreeNode(node->southWest, releaseBlock);
  if (node->southEast != NULL) TBFreeQuadTreeNode(node->southEast, releaseBlock);
...
//  }
  if (releaseBlock != nil) {
    for (int i=0; i < node->count; i++) {
      releaseBlock(node->points[i].data);
    }
  }
  free(node->points);
  free(node);
